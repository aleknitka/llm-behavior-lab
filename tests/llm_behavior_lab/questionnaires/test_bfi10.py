...
def test_bfi10_defines_construct_high_mean_scores() -> None:
    assert [scale.id for scale in BFI_10.scales] == [
        "extraversion",
        "agreeableness",
        "conscientiousness",
        "emotional_stability",
    ...
    assert [
        [mapping.reverse_scored for mapping in scale.item_mappings]
        for scale in BFI_10.scales
    ] == [[False, True], [True, False], [True, False], [True, False], [True, False]]
    assert len(BFI_10.scoring_models) == 1
    scoring_model = BFI_10.scoring_models[0]
    assert scoring_model.id == "default"
    assert scoring_model.provenance == "source_defined"
    assert [rule.scale_id for rule in scoring_model.scale_rules] == [
        scale.id for scale in BFI_10.scales
    ]
    assert all(rule.transformation == "mean" for rule in scoring_model.scale_rules)
