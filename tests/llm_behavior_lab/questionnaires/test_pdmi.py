...
from llm_behavior_lab.questionnaires import BFI_10
...
def test_pdmi_scales_match_source_subscales() -> None:
    ...
    assert {
        ...
        "emotional": [f"pdmi_{index:02d}" for index in range(1, 31)],
        "reasoned": [f"pdmi_{index:02d}" for index in range(31, 51)],
    }
...
def test_pdmi_preserves_representative_factor_loading_metadata() -> None:
    ...
    assert len(questionnaire.scoring_models) == 1
    assert questionnaire.scoring_models[0].id == "default"
    assert [scale.id for scale in questionnaire.scales][-2:] == ["emotional", "reasoned"]
    assert [rule.scale_id for rule in questionnaire.scoring_models[0].scale_rules] == [
        scale.id for scale in questionnaire.scales
    ]


def test_pdmi_readme_exists_and_root_readme_links_to_it() -> None:
    questionnaire_readme = Path("src/llm_behavior_lab/questionnaires/pdmi/README.md")
