...
from llm_behavior_lab.models import Persona
...
def render_persona_intro(persona: Persona, context: str | None = None) -> str:
    ...
    Args:
        ...
        context: Optional supplemental text the model should read before
            responding to the procedure, such as an experimental vignette,
            product description, or other paragraph-level stimulus. Blank or
            whitespace-only values are treated as missing context.

    Returns:
        Rendered prompt text suitable for the first message in an experiment.
    ...
    Examples:
        ...
        True
    """
    template_path = files("llm_behavior_lab.prompts").joinpath("persona_intro.j2")
