...
def _scales(items: list[Item]) -> list[Scale]:
    ...
    for scale_id, name, item_range, cronbach_alpha in [
        ("emotional", "Emotional purchase decisions", range(1, 31), 0.915),
        ("reasoned", "Reasoned purchase decisions", range(31, 51), 0.919),
    ]:
        scales.append(
            Scale(
                id=scale_id,
                ...
                item_mappings=[
                    ItemMapping(
                        ...
                        metadata={
                            "factor_loading": item.metadata["factor_loading"],
                            "source_scale": item.metadata["source_scale"],
                            "source_subscale": item.metadata["source_subscale"],
                            "source_table": item.metadata["source_table"],
                        },
                    )
                    for item in items
                    if item.order in item_range
                ],
                metadata={
...
_PDMI_ITEMS = _items()
_PDMI_SCALES = _scales(_PDMI_ITEMS)
_PDMI_SCORING_MODEL = ScoringModel(
    id="default",
    name="PDMI mean scores",
    version="1.0",
...
PURCHASE_DECISION_MAKING_INVENTORY = Questionnaire(
    ...
    items=_PDMI_ITEMS,
    scales=_PDMI_SCALES,
    scoring_models=[_PDMI_SCORING_MODEL],
    metadata={
        "retain_history": True,
        "source_doi": PDMI_SOURCE_DOI,
        "source_url": PDMI_SOURCE_URL,
        ...
        "scoring_models_status": "implemented",
    },
